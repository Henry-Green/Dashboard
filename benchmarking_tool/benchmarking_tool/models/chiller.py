...
class Chiller(db.Model):
    ...
    cooling_capacity = db.Column(db.String(200), unique = False, nullable = False)
    type_chiller = db.Column(db.String(200), unique = False, nullable = False)
    kwton = db.Column(db.Float, unique = False, nullable=False)
    eer = db.Column(db.Float, unique =False, nullable=False)
